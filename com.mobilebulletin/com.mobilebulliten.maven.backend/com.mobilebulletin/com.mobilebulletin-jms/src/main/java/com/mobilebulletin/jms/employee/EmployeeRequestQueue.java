...
public class EmployeeRequestQueue implements Runnable
{
	private static final Logger log = Logger.getLogger(EmployeeRequestQueue.class.getName());
	...
	private Object replyObject;
	 ...
	 {		 
		  ...
		  String requestQueueName ="queue/EmployeeMOBBRequestQueue";
		  replyQueueName ="queue/EmployeeMOBBReplyQueue";
	 ...
	 public Object sendMessage() throws RuntimeException
	 {
		 ...
		 try 
		 {
			 ...
			 ObjectMessage objMsg = null;
              ...
              if(object instanceof AddEmployeeRequest)
			{
				AddEmployeeRequest request = (AddEmployeeRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof RemoveEmployeeRequest)
			{
				RemoveEmployeeRequest request = (RemoveEmployeeRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof UpdateEmployeeRequest)
			{
				UpdateEmployeeRequest request = (UpdateEmployeeRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof ImportEmployeesRequest)
			{
				ImportEmployeesRequest request = (ImportEmployeesRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof ImportEmployeesFromDatabaseRequest)
			{
				ImportEmployeesFromDatabaseRequest request = (ImportEmployeesFromDatabaseRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof GetEmployeeByIdRequest)
			{
				GetEmployeeByIdRequest request = (GetEmployeeByIdRequest)object;
				objMsg = session.createObjectMessage(request);
			}else               if(object instanceof GetEmployeesFromCompanyRequest)
			{
				GetEmployeesFromCompanyRequest request = (GetEmployeesFromCompanyRequest)object;
				objMsg = session.createObjectMessage(request);
			}
